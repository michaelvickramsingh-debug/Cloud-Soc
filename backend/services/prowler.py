...
services/prowler.py
--------------------
Prowler CSPM findings ingestion service.

What is Prowler?
  Prowler is an open-source cloud security tool that runs 1,000+ checks
  against real AWS accounts and outputs findings in JSON-OCSF format.
  It specialises in IOM (Indicator of Misconfiguration) findings —
  things like MFA not enabled, public S3 buckets, GuardDuty disabled.

Why integrate Prowler with CloudGuard?
  CloudGuard simulates IOA findings (runtime attack behaviour).
  Prowler provides real IOM findings (actual AWS misconfigurations).
  Together they give CloudGuard dual data sources — exactly what the
  CrowdStrike white paper argues is needed for complete CDR coverage.

How it works:
  1. You run Prowler against your AWS account:
         prowler aws -M json-ocsf -o backend/data/
  2. Prowler writes prowler_output.ocsf.json to backend/data/
  3. You call POST /api/prowler/ingest
  4. This service reads the file, maps each FAIL finding → IOM Alert
  5. Alerts land in the same alerts table as your simulated IOA alerts
  6. The unified dashboard shows both real posture + simulated runtime

Prowler JSON-OCSF key fields used:
  finding_info.title  → alert title
  finding_info.desc   → context for description
  status_code         → only FAIL findings become alerts
  severity            → Critical / High / Medium / Low
  resources[0].name   → resource name
  resources[0].cloud.region → AWS region
  metadata.event_code → maps to MITRE technique where possible
  remediation.desc    → added to alert description
